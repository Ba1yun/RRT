...
class RRT:
    ...
    def __init__(self, obstacleList, randArea,
                 expandDis=2.0, goalSampleRate=60, maxIter=200):
    ...
    def get_new_node(self, theta, n_ind, nearestNode):
        ...
        if random.randint(0, 100) > mubiao_rate:
            point1=np.array([self.start.x,self.start.y])
            point2=np.array([newNode.x,newNode.y])
            point3=np.array([self.goal.x,self.goal.y])
            l1=point2-point1
            l2=point3-point1
            cos=(l1*l2)/(np.linalg.norm(l1)*np.linalg.norm(l2))
            p=cos[0]
            if p>k:
                d = self.line_cost(newNode, self.goal)
                newNode.x += (d * math.cos(theta))/3
                newNode.y += (d * math.sin(theta))/3
            else:
                newNode.x += 0
                newNode.x += 0
        else:
    ...
    def draw_graph(self, rnd=None, path=None):
        plt.clf()
        matplotlib.pyplot.figure(0, figsize = (8, 6))
        matplotlib.pyplot.xlim(-2,18)
        matplotlib.pyplot.ylim(-2,16)
        matplotlib.pyplot.title(u"path planning",fontsize=16)


        
        plt.xlabel(u'x')
        plt.ylabel(u'y')
        #plt.title(u"规划曲线图",fontproperties=my_font,fontsize=16)
        # for stopping simulation with the esc key.
        plt.gcf().canvas.mpl_connect(
            'key_release_event',
            lambda event: [exit(0) if event.key == 'escape' else None])
        if rnd is not None:
            matplotlib.pyplot.plot(rnd.x, rnd.y, "^k")
        for node in self.node_list:
            if node.parent is not None:
                if node.x or node.y is not None:
                    matplotlib.pyplot.plot([node.x, self.node_list[node.parent].x], [
                        node.y, self.node_list[node.parent].y], "-g")
        for (ox, oy, size) in self.obstacle_list:
            # self.plot_circle(ox, oy, size)
            matplotlib.pyplot.plot(ox, oy, "ok", ms=30 * size)

        matplotlib.pyplot.plot(self.start.x, self.start.y, "xr")
        matplotlib.pyplot.plot(self.goal.x, self.goal.y, "xr")
       

        if path is not None:
            matplotlib.pyplot.figure(1, figsize = (20, 20))
            matplotlib.pyplot.title(u"path planning",fontsize=16)
            plt.xlabel(u'x',fontsize=20)
            plt.ylabel(u'y',fontsize=20)
            matplotlib.pyplot.xlim(-2,18)
            matplotlib.pyplot.ylim(-2,16)
            matplotlib.pyplot.plot([x for (x, y) in path], [y for (x, y) in path],  '-y',
                                   label='primitive curve')
            for (ox, oy, size) in self.obstacle_list:
                matplotlib.pyplot.plot(ox, oy, "ok", ms=30 * size)
            matplotlib.pyplot.plot(self.start.x, self.start.y, "xr")
            matplotlib.pyplot.plot(self.goal.x, self.goal.y, "xr")
            ...
            func =interpolate.interp1d(xx,yy,kind='quadratic')
            ynew =func(xnew)
            matplotlib.pyplot.plot(xnew,ynew,c='b',label='Lagrange difference curve')
...
def main():
   ...
   # obstacleList = [
    ...
    path = rrt.rrt_planning(start=[0, 2], goal=[13, 12], animation=show_animation)
    ...
    #func =interpolate.interp1d(bb,cc,kind='slinear')
    #ynew =func(xnew)
    #plt.plot(xnew,ynew)
    #print(ynew)
    print("Done!!")
